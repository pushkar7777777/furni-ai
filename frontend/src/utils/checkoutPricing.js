...
export const paymentMethods = [
  { id: 'upi', label: 'UPI', detail: 'PhonePe, Google Pay, Paytm, BHIM', online: true },
  { id: 'card', label: 'Credit / Debit Card', detail: 'Visa, Mastercard, RuPay, Amex', online: true },
  { id: 'netbanking', label: 'Net Banking', detail: 'All major Indian banks', online: true },
  { id: 'wallet', label: 'Wallets', detail: 'Paytm, Mobikwik, Amazon Pay', online: true },
  { id: 'emi', label: 'EMI', detail: 'No-cost and standard EMI options', online: true },
  { id: 'cod', label: 'Cash on Delivery', detail: 'Pay when furniture reaches you', online: false },
];

export const estimateDelivery = ({ cartItems = [], subtotal = 0, city = '', state = '', postalCode = '', paymentMethod = 'upi' }) => {
  const cityKey = city.trim().toLowerCase();
  const profile = cityProfiles[cityKey];
  const postalDistance = getDistanceFromPostalCode(postalCode);
  const distanceKm = profile?.distanceKm || postalDistance || 950;
  ...
  const remoteCharge = state.trim() && !profile && distanceKm > 900 ? 300 : 0;
  const codCharge = paymentMethod === 'cod' ? 49 : 0;
  const shippingCharge = Math.max(0, base + distanceCharge + handlingCharge + remoteCharge + codCharge);

  return {
    shippingCharge: subtotal >= 75000 ? 0 : shippingCharge,
    distanceKm,
    zone,
    eta,
    handlingCharge,
    codCharge,
    factors: [
      `${Math.round(distanceKm)} km from Pune warehouse`,
      `${zone} delivery zone`,
      `${quantity} item${quantity === 1 ? '' : 's'}`,
      heavyItemCount ? `${heavyItemCount} bulky item${heavyItemCount === 1 ? '' : 's'}` : 'standard handling',
    ],
  };
};
